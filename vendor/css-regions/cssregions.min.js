...
function parseerror(s, msg) {
	console.trace("Parse error at token " + s.i + ": " + s.token + ".\n" + msg);
	return true;
}
				...
				try {
					...
					xhr.onreadystatechange = function() {
						if(this.readyState==4) { 
							console.log(this.responseText.slice(this.responseText.indexOf("__vite__css = \"") + "__vite__css = \"".length, this.responseText.indexOf("\"\n__vite__"))
                            .replace("\\n", "\n").replace('\\"', '"'));
							// status 0 is a webkit bug for local files
							if(this.status==200||this.status==0) {
								cssCascade.loadStyleSheet(this.responseText.startsWith("import { createHotContext as __vite__createHotContext }") ?
                                 this.responseText.slice(this.responseText.indexOf("__vite__css = \"") + "__vite__css = \"".length, this.responseText.indexOf("\"\n__vite__"))
                                    .replace("\\n", "\n").replace('\\"', '"')
                                 : this.responseText, this.ruleIndex)
							} else {
