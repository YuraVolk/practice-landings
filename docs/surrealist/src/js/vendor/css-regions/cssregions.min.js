...
!(function () {
	...
	module.exports = (function (window, document) {
		...
		var domEvents = {
			...
			EventTarget: {
				implementsIn: function (eventClass, static_class) {
					if (!static_class && typeof eventClass == "function")
						eventClass = eventClass.prototype;

					eventClass.dispatchEvent =
						domEvents.EventTarget.prototype.dispatchEvent;
					eventClass.addEventListener =
						domEvents.EventTarget.prototype.addEventListener;
					eventClass.removeEventListener =
						domEvents.EventTarget.prototype.removeEventListener;
				},
				prototype: {},
			},
		};
		...
		domEvents.EventTarget.prototype.removeEventListener = function (
			eventType,
			f,
		) {
			if (!this.eventListeners) this.eventListeners = [];
		...
		domEvents.EventTarget.prototype.dispatchEvent = function (event_or_type) {
			...
			if (typeof event_or_type == "string") {
				if (
					...
					this.eventListeners[event_or_type].length == 0
				) {
					return;
				}
			} else {
			...
			if (typeof event == "object") {
				...
				throw new Error(
					"dispatchEvent expect an Event object or a string containing the event type",
				);
			}
	...
	module.exports = (function (window, document) {
		...
		function hexdigit(code) {
			return (
				digit(code) || between(code, 0x41, 0x46) || between(code, 0x61, 0x66)
			);
		}
		...
		function parseerror(s, msg) {
			console.trace(
				"Parse error at token " + s.i + ": " + s.token + ".\n" + msg,
			);
			return true;
		}
		function donothing() {
		...
		function consumeAListOfDeclarations(s) {
			...
			while (s.consume()) {
				if (
					s.token instanceof WhitespaceToken ||
					s.token instanceof SemicolonToken
				) {
					donothing();
				} else if (s.token instanceof EOFToken) {
					return decls;
					...
					while (
						!(
							s.next() instanceof SemicolonToken || s.next() instanceof EOFToken
						)
					)
		...
		AtRule.prototype.toCSSString = function () {
			if (this.value) {
				return (
					"@" +
					escapeIdent(this.name) +
					" " +
					this.prelude.toCSSString() +
					this.value.toCSSString()
				);
			} else {
				return (
					"@" + escapeIdent(this.name) + " " + this.prelude.toCSSString() + "; "
				);
			}
		};
		AtRule.prototype.toStylesheet = function () {
			return (
				this.asStylesheet ||
				(this.asStylesheet = this.value
					? parseAStylesheet(this.value.value)
					: new Stylesheet())
			);
		};
		...
		cssSyntax.parseACommaSeparatedListOfComponentValues =
	...
	module.exports = (function (window, document) {
		...
		function EventStream(connect, disconnect, reconnect) {
			...
			var schedule = (this.schedule = function (newCallback) {
				...
				if (isConnected && !shouldDisconnect) {
					throw new Error(
						"Cannot schedule on an already-scheduled event stream",
					);
				}
		...
		if ("MutationObserver" in window) {
			DOMUpdateEventStream = function DOMUpdateEventStream(options) {
				...
				if (options) {
					var target =
						"target" in options ? options.target : document.documentElement;
					var config = {
						...
						characterData:
							"characterData" in options ? !!options.characterData : false,
					};
				} else {
		...
		function querySelectorLive(selector, handler, root) {
			...
			var loop = function loop(eventStream) {
				...
				while (el1 || el2) {
					if (el1 === el2) {
						...
						el2 &&
						/*el1 is after el2*/ (!el1 ||
							(el2.compareDocumentPosition(el1) & (1 | 2 | 8 | 32)) === 0)
					) {
			...
			if (simpleSelector.indexOf(":") == -1) {
				...
				if (DOMUpdateEventStream != AnimationFrameEventStream) {
					...
					var reg =
						/:(target|checked|indeterminate|valid|invalid|in-range|out-of-range|user-error)\b/gi;
					if (reg.test(simpleSelector)) {
						...
						var reg =
							/:(any-link|link|visited|local-link|enabled|disabled|read-only|read-write|required|optional)\b/gi;
						// simplify simpleSelector
						simpleSelector = simpleSelector.replace(reg, "");
					}
	...
	module.exports = (function (window, document) {
		...
		var cssCascade = {
				...
				for (var i = 0; i < selector.length; i++) {
					if (selector[i] instanceof cssSyntax.IdentifierToken) {
						...
						if (selector[++i] instanceof cssSyntax.ColonToken) {
							...
							var nestedPriority = this.computeSelectorPriorityOf(
								selector[i].value,
							);
			...
			//
			findAllMatchingRulesWithPseudo: function findAllMatchingRules(
				element,
				pseudo,
			) {
							...
							if (rule instanceof cssSyntax.StyleRule) {
								...
								for (var sr = subrules.length; sr--; ) {
									...
									if (pseudo) {
										...
										if (
											selectorLow.lastIndexOf("::" + pseudo) ==
											newLength - 1
										) {
											selector = selector.substr(0, newLength - 1);
										} else if (
											selectorLow.lastIndexOf(":" + pseudo) == newLength
										) {
									...
									try {
										if (element.matches) isMatching = element.matches(selector);
											...
											isMatching = element.matchesSelector(selector);
										else if (element.oMatchesSelector)
											isMatching = element.oMatchesSelector(selector);
								...
								rule instanceof cssSyntax.AtRule &&
								rule.name == "media"
							) {
								// visit them
			...
			getDefaultStyleForTag: function getDefaultStyleForTag(tagName) {
				...
				var style = (this.defaultStylesForTag[tagName] =
			...
			//
			getSpecifiedStyle: function getSpecifiedStyle(
				element,
				cssPropertyName,
				matchedRules,
			) {
				...
				var fragmentSource;
				if (
					(fragmentSource = element.getAttribute(
					...
					if (fragmentSource)
						return cssCascade.getSpecifiedStyle(
							fragmentSource,
							cssPropertyName,
						);
				}
				...
				if (element.currentStyle && !window.opera) {
					...
					var bestValue =
						element.myStyle[cssPropertyName] ||
					...
					try {
						if (
							(bestValue =
								element.style.getPropertyValue(cssPropertyName) ||
								element.myStyle[cssPropertyName])
						) {
							return cssSyntax.parseAListOfComponentValues(bestValue);
						}
					...
					var visit = function (rules) {
						for (var i = rules.length; i--; ) {
							...
							if (rules[i] instanceof cssSyntax.StyleRule) {
								...
								for (var j = decls.length - 1; j >= 0; j--) {
									if (decls[j].type == "DECLARATION") {
										if (decls[j].name == cssPropertyName) {
											...
											var currentPriority =
												cssCascade.computeSelectorPriorityOf(rules[i].selector);
								...
								rules[i] instanceof cssSyntax.AtRule &&
								rules[i].name == "media"
							) {
								// visit them
			...
			loadStyleSheetTag: function loadStyleSheetTag(stylesheet, i) {
				...
				if (stylesheet.tagName == "LINK") {
					...
					try {
						...
						xhr.onreadystatechange = function () {
							if (this.readyState == 4) {
								console.log(
									this.responseText
										.slice(
											this.responseText.indexOf('__vite__css = "') +
												'__vite__css = "'.length,
								...
								if (this.status == 200 || this.status == 0) {
									...
									cssConsole.log(
										"css-cascade polyfill failled to load: " + this.href,
									);
								}
							}
						};
						...
						cssConsole.log(
							"css-cascade polyfill failled to load: " + stylesheet.href,
						);
					}
				} else {
			...
			loadAllStyleSheets: function loadAllStyleSheets() {
				...
				var stylesheets = head.querySelectorAll(
			...
			//
			startMonitoringProperties: function startMonitoringProperties(
				properties,
				handler,
			) {
			...
			splitRule: function splitRule(rule) {
				...
				for (var i = 0; i < rule.selector.length; i++) {
					if (
						rule.selector[i] instanceof cssSyntax.DelimToken &&
						rule.selector[i].value == ","
					) {
				...
				for (var i = 0; i < rules.length; i++) {
					...
					querySelectorLive(rule.selector.toCSSString(), {
						...
						onremoved: function (e) {
							...
							if (e.myMatchedRules)
								e.myMatchedRules.splice(e.myMatchedRules.indexOf(rule), 1);
			...
			polyfillStyleInterface: function (cssPropertyName) {
				var prop = {
					...
					set: function (v) {
						...
						if (
							this.parentElement.getAttribute(
								"data-style-" + cssPropertyName,
							) != v
						) {
				...
				for (var i = styleProtos.length; i--; ) {
					...
					Object.defineProperty(
						styleProto,
						cssCascade.toCamelCase(cssPropertyName),
						prop,
					);
				}
	...
	module.exports = (function (window, document) {
		...
		var cssBreak = {
			...
			isReplacedElement: function isReplacedElement(element) {
				...
				var replacedElementTags =
					/^(SVG|MATH|IMG|VIDEO|PICTURE|OBJECT|EMBED|IFRAME|TEXTAREA|BUTTON|INPUT)$/; // TODO: more
			...
			isSingleLineOfTextComponent: function (
				...
				if (typeof elementDisplay == "undefined")
				...
				if (typeof isReplaced == "undefined")
					isReplaced = this.isReplacedElement(element);
			...
			hasAnyInlineFlow: function (element) {
				function countAsInline(element) {
					if (!(element instanceof Element))
						return !/^\s*$/.test(element.nodeValue);
					return (
						!cssBreak.isOutOfFlowElement(element) &&
						cssBreak.isSingleLineOfTextComponent(element)
					);
				}
			...
			//
			isLineBreakingElement: function (
				element,
				elementStyle,
				elementDisplay,
				elementPosition,
			...
			// (the case of block elements, mostly)
			//
			isLinePreBreakingElement: function (
				element,
				elementStyle,
				elementDisplay,
				elementPosition,
				...
				var breakBefore = cssCascade
					.getSpecifiedStyle(element, "break-before")
					.toCSSString();
				return (
					breakBefore == "region" ||
					breakBefore == "all" ||
					cssBreak.isLineBreakingElement(
						element,
						elementStyle,
						elementDisplay,
						elementPosition,
			...
			// (the case of block elements, mostly)
			//
			isLinePostBreakingElement: function (
				element,
				elementStyle,
				elementDisplay,
				elementPosition,
				...
				var breakAfter = cssCascade
					.getSpecifiedStyle(element, "break-after")
					.toCSSString();
				return (
					breakAfter == "region" ||
					breakAfter == "all" ||
			...
			areInSameSingleLine: function areInSameSingleLine(element1, element2) {
				...
				var firstElement =
					getComputedStyle(element1.parentNode).direction == "rtl"
						? element2box
						: element1box;
				var secondElement =
					getComputedStyle(element1.parentNode).direction == "rtl"
			...
			//
			isHiddenOverflowing: function isHiddenOverflowing(
				element,
				elementOverflow,
			) {
				if (!(element instanceof Element)) return false;
				if (typeof elementOverflow == "undefined")
					elementOverflow = getComputedStyle(element).display;
			...
			isMonolithic: function isMonolithic(element) {
				...
				var isHiddenOverflowing = this.isHiddenOverflowing(
					element,
					elementOverflow,
				);
			...
			isPossibleBreakPoint: function isPossibleBreakPoint(r, region) {
				...
				while (ancestor && lastAncestor !== region) {
					if (lastAncestor && lastAncestor.previousSibling) {
						if (
							this.areInSameSingleLine(
								lastAncestor,
								lastAncestor.previousSibling,
							)
						) {
							return false;
						}
			...
			var TextRangeUtils = {
				...
				convertFromDOMRange: function (domRange) {
					var adoptEndPoint = function (textRange, domRangeInner, bStart) {
						...
						var container =
							domRangeInner[bStart ? "startContainer" : "endContainer"];
						var offset = domRangeInner[bStart ? "startOffset" : "endOffset"],
							textOffset = 0;
						var anchorNode = DOMUtils.isDataNode(container)
							? container
							: container.childNodes[offset];
						var anchorParent = DOMUtils.isDataNode(container)
						...
						textRange.setEndPoint(
							bStart ? "StartToStart" : "EndToStart",
							cursor,
						);
						textRange[bStart ? "moveStart" : "moveEnd"](
							"character",
							textOffset,
						);
					};
	...
	Range.prototype.myMoveOneCharLeft = function () {
		...
		if (r.endOffset > 0) {
			...
			if (previousSibling && previousSibling.lastChild) {
				...
				previousSibling &&
				previousSibling.nodeType == previousSibling.TEXT_NODE
			) {
				// todo: lookup value
	...
	Node.contains = function contains(parentNode, node) {
		if (node.nodeType != 1) {
			...
			return (
				node.parentNode == parentNode || parentNode.contains(node.parentNode)
			);
		} else {
			...
			markNodesAsRegion: function (nodes, fast) {
				nodes.forEach(function (node) {
					...
					node.cssRegionsWrapper =
						node.cssRegionsWrapper ||
						node.appendChild(document.createElement("cssregion"));
				});
			},
				...
				function visit(node, k) {
					...
					switch (node.nodeType) {
						case 1: // Element node
							...
							// expand list values
							if (node.tagName == "OL")
								cssRegionsHelpers.expandListValues(node);
						...
						case 3: // Text node
							if (
								...
								if (
									...
									if (
										cssRegionsHelpers.listOfTextNodesForIE.indexOf(node) == -1
									) {
				...
				function visit(node, k) {
					...
					switch (node.nodeType) {
						...
						case 1: // Element node
							...
							if (node.currentStyle) node.currentStyle.display.toString(); // IEFIX FOR BAD STYLE RECALC
							if (node.tagName == "OL")
								cssRegionsHelpers.unexpandListValues(node);
							if (typeof k != "undefined" && node.tagName == "LI")
					...
					switch (node.nodeType) {
						case 3: // Text node
							...
							if (!node.previousSibling) {
								...
								if (
									getComputedStyle(node.parentNode).whiteSpace.substring(
										0,
										3,
									) !== "pre"
								) {
				...
				function visit(node1, node2, isRoot) {
					...
					switch (node1.nodeType) {
						case 1: // Element node
							...
							var properties =
								cssCascade.allCSSProperties || cssCascade.getAllCSSProperties();
							for (var p = properties.length; p--; ) {
								...
								if (
									...
									var style = getComputedStyle(node1).getPropertyValue(
										properties[p],
									);
								...
								var cssValue = cssCascade.getSpecifiedStyle(
									node1,
									properties[p],
									matchedRules,
								);
								if (cssValue && cssValue.length) {
									...
									if (!(properties[p] in cssCascade.inheritingProperties))
										continue;
							...
							var importPseudo = function (node1, node2, pseudo) {
								...
								//
								var mayExist = !!cssCascade.findAllMatchingRulesWithPseudo(
									node1,
									pseudo.substr(1),
								).length;
								if (!mayExist) return;
								...
								if (pseudoStyle.content != "none") {
									...
									var node2style = "";
									var properties =
										cssCascade.allCSSProperties ||
										cssCascade.getAllCSSProperties();
									for (var p = properties.length; p--; ) {
	...
	module.exports = function (window, document, cssRegions) {
		...
		cssRegions.Flow.prototype.addToContent = function (element) {
			...
			if (
				content.length == 0 ||
				content[content.length - 1].nextSibling === element
			) {
		...
		cssRegions.Flow.prototype.addToRegions = function (element) {
			...
			if (treeWalker.nextNode()) {
				// insert the element at his current location
				regions.splice(
					this.regions.indexOf(treeWalker.currentNode),
					0,
					element,
				);
			} else {
		...
		cssRegions.Flow.prototype._relayout = function (data) {
			...
			try {
				...
				var docElmScrollTop =
					data && data.docElmScrollTop
						? data.docElmScrollTop
						: document.documentElement.scrollTop;
				var docBdyScrollTop =
					data && data.docBdyScrollTop
						? data.docBdyScrollTop
				...
				cssRegions.layoutContent(regionStack, contentFragment, {
					...
					ondone: function onLayoutDone(overset) {
						...
						while (This.regions[This.firstEmptyRegionIndex]) {
							...
							isEmpty =
								isEmpty ||
								!This.regions[This.firstEmptyRegionIndex].cssRegionsWrapper;
							isEmpty =
								isEmpty ||
								!This.regions[This.firstEmptyRegionIndex].cssRegionsWrapper
									.firstChild;
						...
						if (This.regions.length > 0 && !This.restartLayout) {
							...
							if (isBuggy) {
								...
								cssConsole.warn(
									"Buggy css regions layout: the page changed; we need to restart.",
								);
								This.restartLayout = true;
							} else {
		...
		cssRegions.Flow.prototype.addEventListenersTo = function (nodes) {
			...
			nodes.forEach(function (element) {
				if (!element.cssRegionsEventStream) {
					element.cssRegionsEventStream = new ES.DOMUpdateEventStream({
						target: element,
					});
		...
		cssRegions.NamedFlow.prototype.getRegionsByContent =
			function getRegionsByContent(node) {
				var regions = [];
		...
		cssRegions.NamedFlowCollection.prototype.namedItem = function (k) {
			return (
				cssRegions.flows[k] || (cssRegions.flows[k] = new cssRegions.Flow(k))
			);
		};
		...
		cssRegions.enablePolyfillObjectModel = function () {
			...
			document.getNamedFlows = function () {
				...
				for (var flowName in cssRegions.flows) {
					if (Object.prototype.hasOwnProperty.call(flows, flowName)) {
						...
						if (
							flows[flowName].content.length != 0 ||
							flows[flowName].regions.length != 0
						) {
			...
			document.getNamedFlow = function (flowName) {
				...
				return (
					flows[flowName] ||
					(flows[flowName] = new cssRegions.NamedFlow(flowName))
				);
			};
		...
		var cssRegions = {
			...
			layoutContent: function (regions, remainingContent, callback, startTime) {
				...
				while (true) {
					...
					if (
						regionDisplay == "none" ||
						regionDisplay.indexOf("inline") !== -1
					) {
				...
				if (!remainingContent.hasChildNodes()) {
					region.cssRegionHost.cssRegionsLastOffsetHeight =
						region.cssRegionHost.offsetHeight;
					region.cssRegionHost.cssRegionsLastOffsetWidth =
						region.cssRegionHost.offsetWidth;
			...
			layoutContentInNextRegionsWhenReady: function (
				...
				for (var imgs_index = imgs.length; imgs_index--; ) {
					if (
						!imgs[imgs_index].complete &&
						!imgs[imgs_index].hasAttribute("height")
					) {
						return setTimeout(
							function () {
				...
				var regionDidOverflow =
					region.cssRegionHost.scrollHeight !=
					region.cssRegionHost.offsetHeight;
					...
					while (current) {
						if (current != first) {
							if (
								/(region|all|always)/i.test(
									cssCascade
										.getSpecifiedStyle(current, "break-before", undefined, true)
										.toCSSString(),
								)
							) {
						...
						if (current != last) {
							if (
								/(region|all|always)/i.test(
									cssCascade
										.getSpecifiedStyle(current, "break-after", undefined, true)
										.toCSSString(),
								)
							) {
				...
				if (startTime + 200 > Date.now()) {
					return cssRegions.layoutContent(
						regions,
						remainingContent,
						callback,
						startTime,
			...
			layoutContentInLastRegionWhenReady: function (
				...
				if (
					...
					var didOverflow =
						this.extractOverflowingContent(region).hasChildNodes();
			...
			extractOverflowingContent: function (region, dontOptimize) {
				...
				var lostHeight = parseInt(
					getComputedStyle(region.cssRegionHost).paddingBottom,
				);
				lostHeight += parseInt(
					getComputedStyle(region.cssRegionHost).borderBottomWidth,
				);
			...
			cssConsole.log(pos.bottom)*/
				...
				if (
					!r ||
					(region !== r.endContainer && !Node.contains(region, r.endContainer))
				) {
				...
				do {
					...
					while (
						...
						var nextSibling = r.endContainer.childNodes[r.endOffset];
						var nextSiblingRect =
							!nextSibling || Node.getBoundingClientRect(nextSibling);
					...
					if (!dontOptimize) {
						...
						while (
							(current = cssRegionsHelpers.getAllLevelPreviousSibling(
								current,
								region,
							))
						) {
							if (
								Node.getBoundingClientRect(current).bottom >
								pos.top + sizingH
							) {
								r.setStart(region, 0);
								r.setEnd(region, 0);
				...
				if (
					...
					var availBorderHeight = parseFloat(
						endContainerStyle.borderBottomWidth,
					);
				...
				if (r.endOffset == 0 && r.endContainer !== region) {
					...
					var excessHeight =
						availBorderHeight + availPaddingHeight - availHeight;
				...
				for (var i = allAncestors.length - 1; i >= 0; i--) {
					...
					if (getComputedStyle(allAncestors[i]).display.indexOf("block") >= 0) {
						allAncestors[i].setAttribute(
							"data-css-continued-block-fragment",
							true,
						);
					}
				}
				if (typeof borderCut === "number") {
					...
					allAncestors[0].setAttribute(
						"data-css-special-continued-fragment",
						true,
					);
					allAncestors[0].style.borderBottomWidth =
						availBorderHeight - borderCut + "px";
				}
				if (typeof paddingCut === "number") {
					...
					allAncestors[0].style.paddingBottom =
						availPaddingHeight - paddingCut + "px";
				}
				if (typeof topBorderCut === "number") {
					allAncestors[0].removeAttribute("data-css-continued-fragment");
					allAncestors[0].setAttribute("data-css-continued-fragment", true);
					allAncestors[0].style.borderTopWidth =
						availBorderHeight - topBorderCut + "px";
				}
				if (typeof topPaddingCut === "number") {
					...
					allAncestors[0].style.paddingTop =
						availPaddingHeight - topPaddingCut + "px";
				}
				...
				if (tmp && (tmp = tmp.lastChild) && !tmp.tagName && tmp.nodeValue) {
					...
					if (nodeValue) {
						...
						if (/\u00AD$/.test(nodeValue)) {
							...
							overflowingContent &&
							overflowingContent.textContent[0] == "\u00AD"
						) {
							nodeValue = nodeValue + "-";
						}
				...
				var newFragments = overflowingContent.querySelectorAll(
				...
				if (specialNewFragment) {
					specialNewFragment.removeAttribute(
						"data-css-special-continued-fragment",
					);
					...
					if (typeof topBorderCut === "number") {
						...
						specialNewFragment.setAttribute(
				...
				cssCascade.startMonitoringProperties(
					["flow-into", "flow-from", "region-fragment"],
					{
						onupdate: function onupdate(element, rule) {
							...
							if (element.getAttributeNode("data-css-regions-fragment-of"))
								return;
							...
							//
							var flowInto = cssCascade
								.getSpecifiedStyle(element, "flow-into")
								.filter(function (t) {
									return t instanceof cssSyntax.IdentifierToken;
								});

							var flowIntoName = flowInto[0]
								? flowInto[0].toCSSString().toLowerCase()
								: "";
							if (
								flowIntoName == "none" ||
							...
							var flowIntoType = flowInto[1]
								? flowInto[1].toCSSString().toLowerCase()
								: "";
							if (flowIntoType != "content") {
								flowIntoType = "element";
							}
							var flowInto = flowIntoName
								? flowIntoName + " " + flowIntoType
								: "";

							var flowFrom = cssCascade
								.getSpecifiedStyle(element, "flow-from")
								.filter(function (t) {
									return t instanceof cssSyntax.IdentifierToken;
								});

							var flowFromName = flowFrom[0]
								? flowFrom[0].toCSSString().toLowerCase()
								: "";
							if (
								flowFromName == "none" ||
							...
							if (
								...
								var lastFlowFrom =
									cssRegions.flows[element.cssRegionsLastFlowFromName];
								var lastFlowInto =
								...
								if (flowFromName) {
									var lastFlowFrom = (cssRegions.flows[flowFromName] =
										cssRegions.flows[flowFromName] ||
										new cssRegions.Flow(flowFromName));
								...
								if (flowIntoName) {
									var lastFlowInto = (cssRegions.flows[flowIntoName] =
										cssRegions.flows[flowIntoName] ||
										new cssRegions.Flow(flowIntoName));
				...
				window.addEventListener("resize", function () {
					...
					if (!relayoutModifiedFlows.timeout) {
						...
						if (hasOngoingLayouts()) {
							...
							relayoutModifiedFlows.timeout = setTimeout(
								relayoutModifiedFlows,
								500,
							);
						} else {
